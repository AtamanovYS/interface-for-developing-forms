...
// Если реквизит относится к метаданным, то возвращается метаданное
//
// Параметры:
//  Форма - ФормаКлиентскогоПриложения
...
Функция ПолучитьРеквизитПоПути(Форма, Путь)
	...
	Если ПутьРазделенныйТочками.Количество() > 1 И ПутьРазделенныйТочками[0] = "Объект" Тогда
		...
		Иначе
			...
			Если СтруктураВозврата.Родитель.Реквизиты.Найти(ПутьРазделенныйТочками[1]) <> Неопределено Тогда
				СтруктураВозврата.Реквизит = СтруктураВозврата.Родитель.Реквизиты[ПутьРазделенныйТочками[1]];
			Иначе
				РеквизитыТаблицы = Форма.ПолучитьРеквизиты("Объект." + СтруктураВозврата.Родитель.Имя);
				Для Каждого Стр Из РеквизитыТаблицы Цикл
					Если Стр.Имя = ПутьРазделенныйТочками[1] Тогда
	...
	Для Каждого Стр Из МассивРеквизитов Цикл
		Если ПутьРазделенныйТочками.Количество() > 1 И
		  ((Стр.ТипЗначения.Типы().Количество() = 1 И Стр.ТипЗначения.Типы()[0] = Тип("ТаблицаЗначений")) 
		   ИЛИ
		   (Стр.ТипЗначения.Типы().Количество() = 1 И Стр.ТипЗначения.Типы()[0] = Тип("ДеревоЗначений"))) Тогда
