...
Процедура ПоказатьПрогрессВыполнения(Всего, Сч, Знач СтрокаСообщения = "", Шаг = Неопределено, ИмяСобытия = "", Точность = 2) Экспорт
	...
	#Иначе
		...
		КонецЕсли;
		Если Шаг = Неопределено ИЛИ (Сч / Всего * 100) >= Шаг ИЛИ Сч = 1 Тогда 
...
Процедура ПоказатьСостояние(Знач СтрокаСообщения, ИмяСобытия = "", УровеньЖурнала = Неопределено) Экспорт
