...
Функция ДобавитьПолеФормы(Форма, ПутьКданным, Родитель = Неопределено, ЭлементПередКоторымВставить = Неопределено, ВидПоля = Неопределено, Эталон = Неопределено)
	...
	РодительПоля = Поле.Родитель;
	Поле = Поле.Реквизит;         
	ИмяЭлемента = ?(РодительПоля = Неопределено, Поле.Имя, РодительПоля.Имя + Поле.Имя);
	...
	Если ЭтоРеквизитОбъекта Тогда
		Если НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода И Поле.ПроверкаЗаполнения = ПроверкаЗаполнения.ВыдаватьОшибку Тогда
...
//  1) первая буква становится заглавной 
//  2) перед всеми заглавными буквами кроме первой ставится пробел
//  3) все заглавные буквы кроме первой превращаются в обычные
//
// Параметры:
//  Имя - Строка, имя написаное в стиле CamelCase с возможным указанием префикса (например, "дк_ОформитьДокументыПроизводства") 
//  УдалятьПрефикс - Булево, признак удаления префикса
//  РазделительПрефиксаОтИмени - Строка, по данному разделителю удаляется префикс
//  КонкретныйПрефикс - Строка или Неопределено, если значение заполнено, то данный префикс ищется перед разделителем и только тогда префикс удаляется
//
// Возвращаемое значение:
//  Строка - заголовок, сформированный на основании имени
//
Функция ПолучитьЗаголовокПоИмени(Имя, УдалятьПрефикс = Истина, РазделительПрефиксаОтИмени = "_", КонкретныйПрефикс = Неопределено)
	
	ИмяБезПрефикса = Имя;
	Если УдалятьПрефикс Тогда
		Если КонкретныйПрефикс <> Неопределено Тогда
			РазделительПрефиксаОтИмени = КонкретныйПрефикс; 
		КонецЕсли;
		Если Найти(Имя, РазделительПрефиксаОтИмени) > 1 Тогда
			ИмяБезПрефикса = Прав(Имя, СтрДлина(Имя) - Найти(Имя, РазделительПрефиксаОтИмени));		
		КонецЕсли;
	КонецЕсли;
